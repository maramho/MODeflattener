...
def peephole_fold_jcc_skip_jmp(block, forbid_offsets=None):
    """
    패턴:  [Jcc][JMP short $+5][JMP near rel32]
    조건:
      - 두 번째( short ) 길이 2, 세 번째( near ) 길이 5
      - short의 목적지 == near의 '끝 주소' (정확히 5바이트 뒤)
      - forbid_offsets에 있는 near jmp 오프셋은 건드리지 않음
    효과:
      - short/near 두 점프를 함께 NOP (Jcc는 유지 → false면 자연스런 fall-through)
    """
    patches = []
    L = block.lines
    for i in range(len(L) - 2):
        a, b, c = L[i], L[i+1], L[i+2]
        if not (_is_jcc(a) and b.name.lower() == 'jmp' and c.name.lower() == 'jmp'):
            continue
        if not (b.l == 2 and c.l == 5):
            continue
        tgt_b = _branch_target_as_int(b.args[0]) if b.args else None
        end_c = c.offset + c.l
        if tgt_b is None or tgt_b != end_c:
            continue
        if forbid_offsets and (c.offset in forbid_offsets):
            continue
        patches.append(PatchNop(b.offset, b.l))  # jmp $+5
        patches.append(PatchNop(c.offset, c.l))  # jmp rel32
    return patches
...
def deflat(addr, main_asmcfg):
    ...
    # dispatcher로 향하는 직전 분기 지점 (pre_dispatcher) 탐지
    # dispatcher에서 도달 가능한 관련 블록 모듬(relevant block) 수집
    # 어디서 들어오고 어디로 뻗어나가는지 뼈대를 파악 
    state_to_target = build_dispatcher_map(main_asmcfg, dispatcher) 
    # BFS로 비교 노드(같은 블록내 cmp/sub eax,imm 후 je/jz 따라가며) 상태값 -> 타깃 블록 주소 딕셔너리 구성
    # 타깃 블록의 마지막 jmp 가 dispatcher로 복귀하는 패턴도 케이스로 정함
    ...
    for state_val, target_bb in state_to_target.items():
        visited_chain = set()
        cur = target_bb    # 여기 target_Bb 부터 실행 체인을 따라가면서 반복
        ...
        while True:
            ...
            tail_off = _find_tail_jmp_to_dispatcher(block, dispatcher, main_asmcfg) # 디스패처로 복귀하는 jmp 찾기
            forbid = {tail_off} if tail_off is not None else set()
            patches.extend(peephole_fold_jcc_skip_jmp(block, forbid_offsets=forbid))

            # next_state 계산 (컨텍스트 전달)  , 다음 상태 추출 
            ...
            nxt = state_to_target.get(next_state)   # 단일 후보이면 해당 함수 next_state
            if nxt is None:
                # 체인 종결
                _capture_regs_for_next(block, known_regs, known_mem)
                _capture_stack_consts_for_next(block, known_mem, known_regs)
...
if __name__ == '__main__':
    ...
    loglevel = args.log
    _log = setup_logger(loglevel) # setup_logger(loglevel 로 로그 레벨 적용해서 로거/ 타이머 초기화)

    deobf_start_time = time.time()

    forg = open(args.filename, 'rb')
    fpatch = open(args.patch_filename, 'wb') # 원본을 통째로 복사해서 patch_filename에 저장 (여기에 패치를 덮어씌움)
    fpatch.write(forg.read())

    loc_db = LocationDB()   # miasm 환경 구성해서 location DB 생성

    global cont
    cont = Container.from_stream(open(args.filename, 'rb'), loc_db)  # cONTAINERFROM_STREA(...) 로 바이너리를 로드함  cont.arch 확인  x86_32/x86_64만 지원함
    ...
    bin_base_addr = section_ep.addr - section_ep.offset   # 파일 오프셋 <-> 가상 주소 변환용 기준으로 계산
    _log.info('bin_base_addr: %#x' % bin_base_addr)

    machine = Machine(cont.arch)
    mdis = machine.dis_engine(cont.bin_stream, loc_db=loc_db)

    ad = int(args.address, 0)   # 초기 작업 큐 구축   아래 초기화 작업
    todo = [(mdis, None, ad)]
    done = set()
    all_funcs = set()
    all_funcs_blocks = {}

    while todo:    # 여기 todo 에서 주소를 꺼내서 mdis.dis_multiblock(ad)로 ASM CFG 생성
        mdis, caller, ad = todo.pop(0)
        if ad in done:
            continue
        done.add(ad)
        asmcfg = mdis.dis_multiblock(ad)  # 
        lifter = machine.lifter_model_call(mdis.loc_db)   # lifter_model_call로 IR CFG 생성해서  all_funcs_blocks[ad] = (asmcfg, ircfg) 에 저장
        ircfg = lifter.new_ircfg_from_asmcfg(asmcfg)

        _log.info('found func @ %#x (%d)' % (ad, len(all_funcs)))   #찾은 모든 함수에 대해서  CALC_fLATTENING_SCORE 계산
