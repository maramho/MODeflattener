...
from mod_utils import remove_dummy_jmps
...
class PatchNop:
    def __init__(self, offset, size):
        self.offset = offset
        self.data = b'\x90' * size

class PatchJmp:
    def __init__(self, offset, jmp_target):
        self.offset = offset
        self.data = b'\xE9' + (jmp_target - offset - 5).to_bytes(4, byteorder='little', signed=True)
...
def calc_flattening_score(graph):
    ...
    if cnt_blocks == 0:
        return 0.0
    return (cnt_cmp_or_sub + cnt_je_or_jmp) / float(cnt_blocks * 2)
...
def deflat(addr, asmcfg_ircfg_tuple):
    asmcfg, ircfg = asmcfg_ircfg_tuple
    state_to_target = get_state_target_map()
    patches = []
    visited = set()
    ...
    for block in blocks:
        dummy_patches = remove_dummy_jmps(block)
        patches.extend(dummy_patches)
    ...
    for state_val, target_bb in state_to_target.items():
        ...
        block = asmcfg.getby_offset(target_bb)
        
        instrs = block.lines

        print(f"\n[+] Original BB at {hex(target_bb)}:")
        for instr in instrs:
            print(f"    {instr}")
        ...
        if last_instr.name.lower() == "jmp":
            ...
            # ✅ 순서 기반으로 다음 블록 판단
            next_idx = state_val + 1
            next_target = state_to_target.get(next_idx)

            if next_target:
                print(f"[INFO] Inserting jmp to {hex(next_target)}")
                patches.append(PatchJmp(last_instr.offset, next_target))
            else:
                print(f"[WARN] No next block found for state {state_val}, inserting NOP")
                patches.append(PatchNop(last_instr.offset, last_instr.l))

        # ✅ dispatcher로 넘기는 state set mov 제거
        for instr in instrs:
            if instr.name == "mov" and len(instr.args) == 2:
                dst, src = instr.args
                if isinstance(dst, ExprMem) and dst.arg.op == "+":
                    offset_val = dst.arg.args[1].arg
                    if offset_val in (0x34, -0x2C):
...
if __name__ == '__main__':
    ...
    for ad in all_funcs:
        ...
        if not args.all or ad == int(args.address, 0):
            ...
            dummy_patches = remove_dummy_jmps(all_funcs_blocks[ad][0])
            patches.extend(dummy_patches)
