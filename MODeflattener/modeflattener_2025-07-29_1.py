...
def symbolic_dispatcher_solver(binary_path, target_val=5):
    ...
    input_var = claripy.BVS("x", 8 * 10)  # 최대 10바이트까지 허용

    state = proj.factory.full_init_state(
        stdin=angr.SimFileStream(name='stdin', content=input_var, has_end=False)
    )
    ...
    def is_target_state(s):
        try:
            return s.solver.eval(s.regs.eax) == target_val
        except:
            return False
    ...
    if simgr.found:
        found = simgr.found[0]
        input_concrete_bytes = found.solver.eval(input_var, cast_to=bytes)

        print(f"[RESULT] dispatcher_val == {target_val} 에 도달하기 위한 입력값 (bytes): {input_concrete_bytes}")
        return input_concrete_bytes
    else:
        print(f"[FAIL] dispatcher_val == {target_val} 에 도달하지 못했습니다.")
        return None


def dynamic_analysis(binary_path, main_addr, dispatcher_addr=None, log_level=logging.INFO, input_data=b"5\n"):
    import os
    import angr
    ...
    # ✅ 입력값 제공 (예: 5)
    state = proj.factory.full_init_state(
        stdin=angr.SimFileStream(name='stdin', content=input_data)
    )
    ...
    for step in range(1000):
        ...
        try:
            current_input = state.posix.stdin.load(0, 10)
            evaluated_input = state.solver.eval(current_input, cast_to=bytes)
            output_lines.append(f"[DEBUG] 현재 stdin 내용 (앞 10바이트): {evaluated_input}")
        except Exception as e:
            output_lines.append(f"[WARN] stdin 내용 확인 실패: {e}")
...
if __name__ == '__main__':
    ...
    # symbolic_dispatcher_solver를 사용하여 dispatcher_val == 5가 되는 입력 찾기
    print("[*] symbolic_dispatcher_solver 실행 중...")
    target_dispatcher_val = 5
    
    input_concrete = symbolic_dispatcher_solver(args.filename, target_val=target_dispatcher_val)

    # 찾은 값이 있다면 dynamic_analysis에 전달
    if input_concrete is not None:
        input_bytes = f"{input_concrete}\n".encode()
        print(f"[DEBUG] 전달할 input_bytes = {input_bytes}")
        executed_blocks = dynamic_analysis(args.filename, main_addr, log_level="INFO", input_data=input_bytes)
    else:
        print("[!] symbolic_dispatcher_solver 실패 → 기본값 5 사용")
        executed_blocks = dynamic_analysis(args.filename, main_addr, log_level="INFO", input_data=b"5\n")
