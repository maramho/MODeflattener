...
def _mem_key_from_exprmem(mem):
    """
    [RBP + imm] / [RSP + imm] 형태의 ExprMem에서
    ('RBP' or 'RSP', signed_offset) 키를 추출. 맞지 않으면 None.
    """
    if not isinstance(mem, ExprMem):
        return None

    # miasm deprecation 대응: exprmem.ptr 권장, 구버전은 exprmem.arg
    ptr = getattr(mem, "ptr", None)
    if ptr is None:
        ptr = getattr(mem, "arg", None)

    if not isinstance(ptr, ExprOp) or ptr.op != '+':
        return None

    base, off = ptr.args
    if not (isinstance(base, ExprId) and base.name.upper() in ('RBP', 'RSP')):
        return None
    if not isinstance(off, ExprInt):
        return None

    # 기존 유틸에 맞춰 정규화
    offv = _as_int(off)  # 이미 signed 정규화 해주는 헬퍼
    return (base.name.upper(), offv)


def _capture_stack_consts_for_next(block, known_mem, known_regs):
    for ins in block.lines:
        nm = ins.name.lower()
        if nm == 'mov' and len(ins.args) == 2 and isinstance(ins.args[0], ExprMem):
            key = _mem_key_from_exprmem(ins.args[0])
            if key is None: 
                continue
            src = ins.args[1]
            if _is_imm(src):
                known_mem[key] = _as_int(src) & 0xFFFFFFFF
            elif _is_reg(src):
                r = src.name.upper()
                if r in known_regs:
                    known_mem[key] = known_regs[r] & 0xFFFFFFFF
                else:
                    known_mem.pop(key, None)
        # 간단한 in-place 패턴: reg <- [slot]; add/sub; [slot] <- reg
        # 위의 regs 캡처가 먼저 돌기 때문에, 여기선 reg 상수 여부만 보면 됨.
    ...
    for ins in block.lines:
        ...
        if nm == 'mov' and len(ins.args) == 2:
            ...
            if _is_reg(dst) and _is_imm(src):
                known_regs[dst.name.upper()] = _as_int(src)
            elif _is_reg(dst) and isinstance(src, ExprMem):
                key = _mem_key_from_exprmem(src)
                if key is not None and key in known_mem:
                    known_regs[dst.name.upper()] = known_mem[key]
                else:
                    known_regs.pop(dst.name.upper(), None)
            elif isinstance(dst, ExprMem) and _is_reg(src):
                # 아직 실제로 known_mem에 쓰지는 말고, 레지스터를 최신으로 유지
                if src.name.upper() in known_regs:
                    pass
        elif nm == 'xor' and len(ins.args) == 2 and _is_reg(ins.args[0]) and _is_reg(ins.args[1]) and ins.args[0].name == ins.args[1].name:
            known_regs[ins.args[0].name.upper()] = 0
        elif nm in ('add','sub') and len(ins.args) == 2 and _is_reg(ins.args[0]) and _is_imm(ins.args[1]):
            r = ins.args[0].name.upper()
            if r in known_regs:
                v = known_regs[r]
                k = _as_int(ins.args[1])
                known_regs[r] = (v + k) & 0xFFFFFFFF if nm == 'add' else (v - k) & 0xFFFFFFFF
        elif nm == 'inc' and len(ins.args) == 1 and _is_reg(ins.args[0]):
            r = ins.args[0].name.upper()
            if r in known_regs:
                known_regs[r] = (known_regs[r] + 1) & 0xFFFFFFFF
        elif nm == 'dec' and len(ins.args) == 1 and _is_reg(ins.args[0]):
            r = ins.args[0].name.upper()
            if r in known_regs:
                known_regs[r] = (known_regs[r] - 1) & 0xFFFFFFFF
