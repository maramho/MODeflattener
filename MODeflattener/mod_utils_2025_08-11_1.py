...
def _capture_stack_consts_for_next(block, known_mem):
    """
...
def _resolve_expr_to_const(lines, expr, known_mem, base_names=('RBP','RSP')):
    if isinstance(expr, ExprInt):
        return int(expr.arg)

    if isinstance(expr, ExprId):
        val = _backtrack_reg_to_imm(lines, expr)    # ← 로컬 호출
        if val is not None:
            return val
        return None
...
def _decide_cmov(dst_val, src_val, relop, L, R):
    """
    cmov* 결정 유틸:
      dst_val: cmov 대상 레지스터의 기존 값(예: EAX=IMM_A)
      src_val: cmov 소스 레지스터 값(예: ECX=IMM_B)
      relop:   'le','l','ge','g','z','nz' 중 하나
      L, R:    직전 cmp L, R 의 피연산(정수)
    반환: 단일 IMM (int) 또는 None(판정 불가)
    """
    if None in (dst_val, src_val, L, R):
        return None
    if   relop == 'le': return src_val if L <= R else dst_val
    elif relop == 'l':  return src_val if L <  R else dst_val
    elif relop == 'ge': return src_val if L >= R else dst_val
    elif relop == 'g':  return src_val if L >  R else dst_val
    elif relop == 'z':  return src_val if L == R else dst_val
    elif relop == 'nz': return src_val if L != R else dst_val
    return None