...
def get_cff_info(asmcfg):
    ...
    for blk in asmcfg.blocks:
        ...
        blk_preds = asmcfg.predecessors(blk.loc_key)
        preds[offset] = blk_preds
        _log.debug(f"Block @ {hex(offset)} has {len(blk_preds)} preds: {[hex(asmcfg.loc_db.get_location_offset(p)) for p in blk_preds]}")

    # pre-dispatcher is the one with max predecessors
    pre_dispatcher = sorted(preds, key=lambda key: len(preds[key]), reverse=True)[0]
    _log.debug(f"pre_dispatcher = {hex(pre_dispatcher)}")
    
    successors = asmcfg.successors(asmcfg.loc_db.get_offset_location(pre_dispatcher))
    _log.debug(f"successors = {[hex(asmcfg.loc_db.get_location_offset(s)) for s in successors]}")
    
    if not successors:
        raise ValueError(f"[ERROR] No successors found for pre_dispatcher at {hex(pre_dispatcher)}")

    dispatcher = asmcfg.loc_db.get_location_offset(successors[0])


    dispatcher = succs[0]
    dispatcher = asmcfg.loc_db.get_location_offset(dispatcher)

    # relevant blocks = pre_dispatcher에 도달하는 블록들
    relevant_blocks = []
