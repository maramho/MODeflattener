...
def get_cff_info(main_asmcfg, dispatcher):
    ...
    for s in successors:
        ...
        # ✅ 각 je 분기 target에서 다음 상태 저장하는 mov eax, imm 확인
        found = False
        for instr in target_block.lines:
            if instr.name == "mov" and isinstance(dst, ExprMem) and isinstance(dst.arg, ExprOp) and \
                dst.arg.op == "+" and isinstance(dst.arg.args[0], ExprId) and dst.arg.args[0].name == "RBP" and \
                isinstance(dst.arg.args[1], ExprInt) and dst.arg.args[1].arg == -4 and isinstance(src, ExprInt):
                    found = True
