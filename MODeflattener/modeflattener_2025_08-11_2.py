                    ...
                    for j in range(idx-1, max(-1, idx-scan_lim), -1):
                        ...
                        if nm == 'cmp' and len(pj.args) == 2:
                            ...
                            if _is_imm(L):
                                ...
                                L_const = known_regs.get(L.name.upper())
                                if L_const is None:
                            ...
                            if _is_imm(R):
                                ...
                                R_const = known_regs.get(R.name.upper())
                                if R_const is None:
...
def deflat(addr, main_asmcfg):
    ...
    for state_val, target_bb in state_to_target.items():
        ...
        while True:
            ...
            # next_state 계산 (known_mem/known_regs 전달)
            ...
            # direct jump 확정 → state write NOP (필요한 케이스만)
            for ins in instrs:
                if ins.name.lower() == 'mov' and len(ins.args) == 2:
                    dst, src = ins.args
