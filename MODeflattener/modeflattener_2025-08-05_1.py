...
def deflat(ad, func_info):
    ...
    machine = Machine(mapped_arch)

    
    for bb in relevant_blocks:
        _log.info(f"[DEBUG] trying to lift block: {hex(bb)}")

        file_offset = cont.addr_to_offset(bb)
        asm_code = cont.memory.load(bb, 16)

        _log.info(f"[DEBUG] raw bytes @ {hex(bb)} (file offset {hex(file_offset)}): {asm_code.hex()}")
    ...
    for addr in relevant_blocks:
        ...
        loc_db = LocationDB()
        mdis = machine.dis_engine(miasm_container.bin_stream, loc_db=loc_db)  # ✅ 정상 작동
    ...
    for lbl, irblock in viewitems(ircfg.blocks):
        _log.info(f"[DEBUG] visiting block: {lbl}, num_instrs: {len(irblock)}")
        for assignblk in irblock:
            _log.info(f"[DEBUG] checking assign:{assignblk}")
...
if __name__ == '__main__':
    ...
    global cont
    cont = cle.Loader(binary_path, auto_load_libs=False).main_object
    ...
    try:
        segment = list(cont.segments)[0]
        binary_data = cont.memory.load(segment.vaddr, segment.memsize)
    except KeyError:
        start_addrs = [addr for addr, _ in cont.memory._backers]
        end_addrs = [addr + len(data) for addr, data in cont.memory._backers]
        min_valid_addr = min(start_addrs)
        max_valid_addr = max(end_addrs)

        print(f"[DEBUG] Using memory range: {hex(min_valid_addr)} - {hex(max_valid_addr)}")
        binary_data = cont.memory.load(min_valid_addr, max_valid_addr - min_valid_addr)

    print(f"[DEBUG] binary_data length: {len(binary_data)}")  # 추가

    miasm_container = Container.from_stream(BytesIO(binary_data), loc_db)

    print(f"[DEBUG] entry point from CLE: {hex(cont.entry)}")  # 확인용
    ...
    real_arch = cont.arch.name
    mapped_arch = arch_aliases.get(real_arch, real_arch)

    supported_arch = ['x86_32', 'x86_64']
    _log.info("Architecture : %s (mapped to %s)" % (real_arch, mapped_arch))

    if mapped_arch not in supported_arch:
        _log.error("Architecture unsupported : %s" % real_arch)
        exit(1)

    # machine = Machine(mapped_arch)
    

    bin_base_addr = cont.min_addr
    _log.info('bin_base_addr: %#x' % bin_base_addr)
    machine = Machine(mapped_arch)
    mdis = machine.dis_engine(miasm_container.bin_stream, loc_db=loc_db)

    print(f"[DEBUG] entry point from CLE: {hex(cont.entry)}")  # 확인용
    print(f"[DEBUG] cont.mapped_base: {hex(cont.mapped_base)}")  # 추가

    ad = int(args.address, 16)  # ✅ 주소 계산 수정
    print(f"[DEBUG] final disasm start address (ad): {hex(ad)}")  # 추가
    todo = [(mdis, None, ad)]
    done = set()
    all_funcs = set()
    all_funcs_blocks = {}
