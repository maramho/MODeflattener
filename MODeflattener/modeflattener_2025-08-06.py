...
def deflat(ad, func_info):
    ...
    # get flattening info
    relevant_blocks, dispatcher, pre_dispatcher = get_cff_info(main_asmcfg, ad)
    ...
    for bb in relevant_blocks:
        ...
        asm_code = cont.bin_stream.getbytes(bb, 16)  # 🔥여기를 수정함🔥
        if asm_code:
            _log.info(f"[DEBUG] raw bytes @ {hex(bb)}: {asm_code.hex()}")
        else:
            _log.warning(f"[WARNING] cannot read bytes @ {hex(bb)}")
    ...
    for addr in relevant_blocks:
        ...
        if len(var_asg) == 1:
            ...
            _log.warning(f"[WARN] Could not identify tail at block {hex(addr)}")
            _log.warning(f"[INFO] var_asg: {var_asg}, last_blk lines: {len(last_blk.lines)}")
    ...
    for lbl, irblock in viewitems(main_ircfg.blocks):
        for assignblk in irblock:
            ...
            # 조건 분기 비교 검사
            if isinstance(src, ExprOp) and src.op == 'FLAG_EQ_CMP':
                # 비교 대상이 immediate일 때만 처리
                arg = src.args[1]
                if isinstance(arg, ExprInt):
                    cmp_val = int(arg)

                    # val_list 조건 검사
                    if cmp_val in val_list:
                        # irblock의 마지막 instruction 기준으로 branch target 추정
                        try:
                            last_assign_items = irblock[-1].items()
                            if not last_assign_items:
                                continue
                            _, loc_expr = last_assign_items[0]
                            # FLAG_EQ_CMP의 true branch destination
                            true_dst = main_ircfg.loc_db.get_location_offset(loc_expr.src1.loc_key)
                            backbone[hex(cmp_val)] = hex(true_dst)
                            _log.debug(f"[BACKBONE] cmp_val: {cmp_val}, target: {hex(true_dst)}")
                        except Exception as e:
                            _log.warning(f"[WARN] Failed to extract true_dst: {e}")
    ...
    if 'tail' in locals():
        ...
        _log.warning("[WARN] 'tail' not found. Skipping rel_blk_info.pop(tail)")
    ...
    if 'tail' in locals():
        tail_instr = main_asmcfg.getby_offset(tail).lines[-1]
        backbone_start, backbone_end = dispatcher, tail_instr.offset + tail_instr.l
        ...
        _log.warning("[WARN] 'tail' not found. Skipping backbone NOPing.")
