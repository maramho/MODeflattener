...
def is_dispatcher(block):
    ...
    for i in range(len(lines) - 1):
        instr = lines[i]
        next_instr = lines[i + 1]

        if (instr.name == 'SUB' and
            instr.args[0].is_int() and
            instr.args[1].is_id() and instr.args[1].name.lower() == 'eax' and
            next_instr.name == 'JE'):
            sub_je_pairs += 1

    # sub/je 쌍이 2개 이상 있으면 dispatcher로 판단
    return sub_je_pairs >= 2
...
def get_sub_je_targets(block, loc_db):
    """
    sub/je dispatcher에서 각 state(IMM)와 대상 BB를 추출
    """
    targets = []
    lines = block.lines
    for i in range(len(lines)-1):
        ...
        if (instr.name == 'SUB' and next_instr.name == 'JE'):
            imm = instr.args[0].arg  # sub IMM, %eax의 IMM 값
            je_target_expr = next_instr.args[0]

            if isinstance(je_target_expr, ExprLoc):
                loc_key = je_target_expr.loc_key
                je_target_addr = loc_db.get_location_offset(loc_key)
                targets.append((imm, je_target_addr))
                print(f"[SUB-JE PAIR] State: {hex(imm)}, BB: {hex(je_target_addr)}")

    return targets


def extract_flattened_edges(cfg, loc_db):
    print("[*] Extracting Flattened Edges from sub/je Dispatcher:")
    edges = []
    for block in cfg.blocks:
        addr = loc_db.get_location_offset(block.loc_key)
        if is_dispatcher(block):
            print(f"[DISPATCHER FOUND] at 0x{addr:x}")
            state_bb_pairs = get_sub_je_targets(block, loc_db)
            for state, bb_addr in state_bb_pairs:
                edges.append((addr, bb_addr))
    print(f"[*] Total Flattened Edges Extracted: {len(edges)}")
    return edges
