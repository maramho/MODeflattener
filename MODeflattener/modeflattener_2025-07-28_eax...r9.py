...
def dynamic_analysis(binary_path, main_addr, log_level="INFO"):
    import claripy
    import angr
    from angr.sim_type import SimTypeFd
    from angr.storage.file import SimFileStream
    ...
    print(f"[INFO] Main function address: {hex(main_addr)}")

    # 🧪 symbolic input (64bit = 8 bytes)
    sym_arg = claripy.BVS("sym_arg", 8 * 8)
    sym_stdin = SimFileStream(name='stdin', content=sym_arg, has_end=False)
    ...
    state.globals['sym_input'] = sym_arg
    simgr = proj.factory.simulation_manager(state)

    executed_blocks = set()
    seen_states = set()

    def track_executed_block(state):
        executed_blocks.add(state.addr)

    state.inspect.b('instruction', when=angr.BP_BEFORE, action=track_executed_block)
    ...
    while simgr.active and step_count < max_steps:
        for state in simgr.active:
            ...
            try:
                dispatcher_val = state.mem[state.regs.rbp - 8].int.concrete  # var_8 추정
            except:
                dispatcher_val = -1

            # 상태 중복 검사 (pc + dispatcher 값 기준)
            state_id = (pc, dispatcher_val)
            if state_id in seen_states:
                continue
            seen_states.add(state_id)

            # 로그 출력
            print(f"[Step {step_count}] PC = {hex(pc)}")
            print(f"    └─ eax = {state.regs.eax}")
            print(f"    └─ edi = {state.regs.edi}")
            print(f"    └─ esi = {state.regs.esi}")
            print(f"    └─ ecx = {state.regs.ecx}")
            print(f"    └─ edx = {state.regs.edx}")
            print(f"    └─ dispatcher var (rbp-0x8) = {dispatcher_val}")

            try:
                alt_disp = state.mem[state.regs.rbp - 4].int.concrete
                print(f"    └─ dispatcher alt (rbp-0x4) = {alt_disp}")
            except:
                pass
            ...
            if state.history.bbl_addrs.hardcopy:
                print(f"    └─ prev block = {hex(state.history.bbl_addrs.hardcopy[-1])}")
    ...
    with open("dh_cleaned_flow.txt", "w") as f:
        for addr in sorted(executed_blocks):
            f.write(f"{hex(addr)}\n")

    return list(executed_blocks)
