...
def dynamic_analysis(binary_path, input_values):
    ...
    for sym in proj.loader.main_object.symbols:
        if 'scanf' in sym.name or '__isoc99_scanf' in sym.name:
            scanf_addr = sym.rebased_addr
            print(f"[INFO] Found scanf symbol: {sym.name} at 0x{scanf_addr:x}")
            break

    if scanf_addr is None:
        print("[ERROR] Could not find scanf address")
        return set()

    proj.hook(scanf_addr, ScanfHook())

    # 🟢 main 함수 주소
    main_func = proj.loader.find_symbol("main")
    main_addr = main_func.rebased_addr if main_func else proj.entry
    print(f"[INFO] Main function address: 0x{main_addr:x}")

    executed_blocks = set()

    for input_val in input_values:
        print(f"\n[INFO] Testing input: {input_val}")
        input_data = (str(input_val) + "\n").encode()
        sim_stdin = angr.SimFileStream(name="stdin", content=input_data)
        ScanfHook.input_value = input_val

        state = proj.factory.call_state(main_addr, stdin=sim_stdin)
        ...
        for a in simgr.active:
            executed_blocks.update(a.history.bbl_addrs)
            try:
                output = a.posix.dumps(1).decode("utf-8", errors="ignore")
                print(f"[INFO] Output: {output.strip()}")
            except:
                pass

        print(f"[INFO] Executed Blocks: {sorted(executed_blocks)}")

    print(f"\n[INFO] All Done. Total blocks: {len(executed_blocks)}")
